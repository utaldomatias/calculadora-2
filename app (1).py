...
def generate_summary(data: Dict, exchange_rate: float, tax_rate: float = 13.83, lambda_payment_option: str = "No Upfront 12x pela AWS", fargate_payment_option: str = "No Upfront 12x pela AWS") -> str:
    ...
    for region in sorted(data['regions']):
        ...
        for service_type in ['EC2', 'RDS', 'ElastiCache', 'CloudFront', 'Lambda', 'Fargate']:
            ...
            # Separar por tipo de pagamento (não misturar On Demand com Reserved)
            ...
            # Calcular totais APENAS se não for mistura de tipos
            no_upfront_cost = sum(i['cost'] for i in no_upfront_instances)
            all_upfront_cost = sum(i['cost'] for i in all_upfront_instances)
            
            # Só somar aos totais se houver instâncias do tipo específico
            if no_upfront_instances:
                total_no_upfront += no_upfront_cost
            
            if all_upfront_instances:
            ...
            if service_type == 'EC2':
                ...
                # Agrupar instâncias por tipo, specs e payment_mode
                grouped_instances = {}
                for instance in instances:
                    ...
                    key = f"{instance['tipo']} ({pricing_strategy} {payment_mode}, {os_system})"
                    
                    if key in grouped_instances:
                        grouped_instances[key] += instance['quantidade']
                    else:
                        grouped_instances[key] = instance['quantidade']
                
                for instance_key, total_qty in grouped_instances.items():
                ...
                for instance in instances:
                    ...
                    key = f"{instance['tipo']} ({az}, {payment_mode}, {period}, {engine})"
                ...
                for instance in instances:
                    ...
                    key = f"{instance['tipo']} ({payment_mode}, {period}, {cache_engine})"
                ...
                for instance in instances:
                    ...
                    config_key = f"{instance['tipo']} ({os_system} {architecture})"
                    
                    if config_key in grouped_fargate:
                        grouped_fargate[config_key] += instance['quantidade']
                    else:
                        grouped_fargate[config_key] = instance['quantidade']
                
                summary += "Configurações:\n"
                for config, qty in grouped_fargate.items():
                    summary += f"-{qty} tarefas/dia - {config}\n"
        ...
        try:
            ...
            for region_services in data['services_by_region'].values():
                for service_type, instances in region_services.items():
                    ...
                    if no_upfront_instances:
                        total_no_upfront += no_upfront_cost * 12  # Converter para anual
            ...
            comparison_data = {
                'Tipo de Pagamento': ['On Demand', 'No Upfront', 'All Upfront'],
                'Custo Anual (USD)': [f"${on_demand_cost:,.2f}", f"${total_no_upfront:,.2f}", f"${total_all_upfront:,.2f}"],
            ...
            if on_demand_cost > 0:
                if total_no_upfront > 0:
                    no_upfront_savings = ((on_demand_cost - total_no_upfront) / on_demand_cost) * 100
            ...
            with col2:
                ...
                if on_demand_cost > 0 and total_all_upfront > 0:
                    main_savings = ((on_demand_cost - total_all_upfront) / on_demand_cost) * 100
            ...
            with st.expander("🔍 Dados Processados (Debug)"):
                ...
                st.write(f"No Upfront Total: ${total_no_upfront:,.2f}")
                st.write(f"All Upfront Total: ${total_all_upfront:,.2f}")
