...
def generate_summary(data: Dict, exchange_rate: float, tax_rate: float = 13.83, lambda_payment_option: str = "No Upfront 12x pela AWS", fargate_payment_option: str = "No Upfront 12x pela AWS") -> str:
    ...
    # Combinar todos os custos No Upfront (EC2, RDS, ElastiCache, CloudFront)
    combined_no_upfront_total = total_no_upfront + total_database_no_upfront
    combined_all_upfront_total = total_all_upfront + total_database_all_upfront
    
    # Resumo financeiro All Upfront (todos os serviços)
    if combined_all_upfront_total > 0:
        summary += "Resumo financeiro All Upfront:\n"
        all_upfront_taxes = combined_all_upfront_total * (tax_rate / 100)
        all_upfront_with_taxes = combined_all_upfront_total + all_upfront_taxes
        all_upfront_brl = all_upfront_with_taxes * exchange_rate
        all_upfront_parcela = all_upfront_brl / 6
        
        summary += f"Valor total (sem imposto): USD {combined_all_upfront_total:,.2f}/ano\n"
    ...
    # Resumo financeiro No Upfront (todos os serviços)
    if combined_no_upfront_total > 0:
        summary += "Resumo financeiro No Upfront:\n"
        no_upfront_annual = combined_no_upfront_total * 12
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # Totais combinados (separando serverless dos outros)
            combined_traditional_no_upfront = total_no_upfront_annual + total_database_no_upfront_annual
            combined_traditional_all_upfront = total_all_upfront + total_database_all_upfront
            combined_total_no_upfront = combined_traditional_no_upfront + total_serverless_no_upfront_annual
            combined_total_all_upfront = combined_traditional_all_upfront + total_serverless_all_upfront
            ...
            comparison_data = {
                'Tipo de Pagamento': ['On Demand', 'No Upfront (Tradicional)', 'All Upfront (Tradicional)', 'No Upfront (Serverless)', 'All Upfront (Serverless)', 'Total No Upfront', 'Total All Upfront'],
                'Custo Anual (USD)': [
                    f"${on_demand_cost:,.2f}",
                    f"${combined_traditional_no_upfront:,.2f}" if combined_traditional_no_upfront > 0 else "$0.00",
                    f"${combined_traditional_all_upfront:,.2f}" if combined_traditional_all_upfront > 0 else "$0.00",
                    ...
                    f"${combined_total_no_upfront:,.2f}" if combined_total_no_upfront > 0 else "$0.00",
                    f"${combined_total_all_upfront:,.2f}" if combined_total_all_upfront > 0 else "$0.00"
                ],
            ...
            # Calcular economias
            if on_demand_cost > 0:
                if combined_traditional_no_upfront > 0:
                    traditional_no_upfront_savings = ((on_demand_cost - combined_traditional_no_upfront) / on_demand_cost) * 100
                    comparison_data['Economia vs On Demand'][1] = f"{traditional_no_upfront_savings:.1f}%"
                
                if combined_traditional_all_upfront > 0:
                    traditional_all_upfront_savings = ((on_demand_cost - combined_traditional_all_upfront) / on_demand_cost) * 100
                    comparison_data['Economia vs On Demand'][2] = f"{traditional_all_upfront_savings:.1f}%"
                ...
                if total_serverless_no_upfront_annual > 0:
                    ...
                    comparison_data['Economia vs On Demand'][3] = f"{serverless_no_upfront_savings:.1f}%"
                
                if total_serverless_all_upfront > 0:
                    serverless_all_upfront_savings = ((on_demand_cost - total_serverless_all_upfront) / on_demand_cost) * 100
                    comparison_data['Economia vs On Demand'][4] = f"{serverless_all_upfront_savings:.1f}%"
                
                if combined_total_no_upfront > 0:
                    total_no_upfront_savings = ((on_demand_cost - combined_total_no_upfront) / on_demand_cost) * 100
                    comparison_data['Economia vs On Demand'][5] = f"{total_no_upfront_savings:.1f}%"
                
                if combined_total_all_upfront > 0:
                    total_all_upfront_savings = ((on_demand_cost - combined_total_all_upfront) / on_demand_cost) * 100
                    comparison_data['Economia vs On Demand'][6] = f"{total_all_upfront_savings:.1f}%"
            ...
            with col2:
                ...
                if on_demand_cost > 0 and combined_total_all_upfront > 0:
                    main_savings = ((on_demand_cost - combined_total_all_upfront) / on_demand_cost) * 100
            ...
            with st.expander("🔍 Dados Processados (Debug)"):
                ...
                st.write(f"Total No Upfront: ${combined_total_no_upfront:,.2f}")
                st.write(f"Total All Upfront: ${combined_total_all_upfront:,.2f}")
                st.write(f"Traditional No Upfront: ${combined_traditional_no_upfront:,.2f}")
                st.write(f"Traditional All Upfront: ${combined_traditional_all_upfront:,.2f}")
